...
def test_identity_matrix_with_bias():
    ...
    expected_lower = np.array([2.0 , 0.5])

    expected_upper = np.array([4.0 , 1.5])
    verifier = IBPVerifier(weights=W, bias=b)
...
def test_multiple_outputs():
    ...
    expected_lower = np.array([1.5, 0.5])
    expected_upper = np.array([6.5, 5.5])
...
def test_multiple_inputs():
    ...
    expected_lower = np.array([2.0, -5.0])
    expected_upper = np.array([4.0, -1.0])
